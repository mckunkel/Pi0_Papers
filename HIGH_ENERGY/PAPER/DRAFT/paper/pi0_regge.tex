...
\usepackage{amsmath}
\captionsetup{justification=raggedright,font=small,margin=0.5cm,singlelinecheck = false}
\usepackage{setspace}
...
photoproduction cross section, via \textcolor{red}{the $\pi^0$ Dalitz decay and the $\pi^0\to\gamma\gamma$ decay in which one $\gamma$ undergoes $e^+e^-$ pair conversion 
mode. The measurement was performed} on a hydrogen target in a wide kinematic range with the CLAS setup at \textcolor{red}{the}
Thomas Jefferson National Accelerator Facility. The measurement was performed using data from the reaction 
$\gamma p\rightarrow pe^+e^-(\gamma)$ using a 
tagged photon beam spanning an energy interval from the ``resonance'' to 
the ``Regge'' regimes, i.e., photon energies $E_{\textcolor{red}{\gamma}} = 1.25-5.55$~GeV. This new data sample quadrupled the \textcolor{red}{world database for $\pi^{0}$ photoproduction}
above $E_{\textcolor{red}{\gamma}} $ = 2~GeV. Our data favors the Regge pole model and the 
constituent counting rule while disfavoring the Handbag 
model.
%The final 
%state particles $p$,$e^+$, and $e^-$ were detected \textcolor{red}{whereas} the photon was 
%inferred from energy and momentum conservation.
%Our data \textcolor{red}{favors} the Regge pole model and the 
%constituent counting rule while disfavoring the Handbag 
%model.
\end{abstract}
%---------------------------------------------
...
%The model of Laget and 
%collaborators~\cite{Laget:2005be} included u-channel baryon exchange, 
%\textcolor{red}{which dominate at backward angles, and elastic and inelastic unitarity cuts, 
%which dominate around $90^{\circ}$. That model is expected to describe 
%the full angular range ($\theta_{\pi}= 0 \to \pi$), while the other models are 
%applicable for more limited ranges of $t$}~\cite{Goldstein:1973xn,Mathieu:2015eia,
%Donnachie:2015jaa}. 
...
%$\gamma p\rightarrow  p\pi^0$ are compared in Figs.~\ref{fig:scaling}
%and \ref{fig:t_data} with data from previous CLAS
%measurements~\cite{Dugger:2007bt}, and bremsstrahlung DESY, Cambridge
%Electron Accelerator (CEA), and SLAC, and Electron Synchrotron at
%Cornell Univ. experiments~\cite{brem} and lower c.m. energy MAMI A2 measurements~\protect\cite{Adlarson:2015byy}. 
...
\bibitem{Adlarson:2015byy} 
  ...
  \bibitem{KunkelPhD} M.~C.~Kunkel,  Ph. D. Thesis, 2014 
