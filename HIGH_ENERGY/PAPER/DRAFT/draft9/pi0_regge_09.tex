...
\altaffiliation[Now at ]{Institut f\"ur Kernphysik, Forschungszentrum 	J\"ulich 52424 \newline Corresponding author. \newline m.kunkel@fz-juelich.de}%Lines break automatically or can be forced with \\
...
\vskip 0.2in
We report first high precision measurement of \textcolor{blue}{exclusive} $\pi^0$ 
photoproduction cross section in Dalitz decay \textcolor{blue}{and conversion mode} on 
...
from the \textcolor{blue}{5.72}~GeV electrons of CEBAF accelerator, impinged on 
liquid hydrogen target. The experimental details \textcolor{blue}{are} given 
in ~\cite{g12}. The reaction of interest is the photoproduction 
of neutral pions on a hydrogen target $\gamma p\to p\pi^0$, where
the neutral pions were detected via external conversion, $\textcolor{blue}{\pi^0 \rightarrow \gamma \gamma \rightarrow e^+e^-\gamma}$ and subsequent Dalitz decay $\pi^0\to \gamma^\ast
\gamma\to e^+e^-\gamma$. 
\textcolor{blue}{Since the final state contains three charged tracks, $p;e^+;e^-$, running the experiment at high 
current was possible, incontrary to single prong charged track detection, which impose limitations due to trigger and data 
...
fit (shown by red solid line) performed with the \textcolor{blue}{CrystalBall} function plus 3rd 
...
energy of the detected system. The 4-C \textcolor{blue}{fi}t used the $\gamma p\to 
p\pi^+\pi^-$ channel to filter background from double charged 
pion production from single $\pi^0$ production. The 1-C \textcolor{blue}{fit}
was used for the topology of $\gamma p\to pe^+e^-(\gamma)$ to 
fit to a missing final state photon.  The 2-C \textcolor{blue}{fi}t was used for 
the topology of $\gamma p\to pe^+e^-(\gamma)$ to fit to a 
missing \textcolor{blue}{sig}nal state photon but also to constrain the invariant 
...
events. To reduce the background further, a comparison of the \textcolor{blue}{ missing mass squared off of the proton} and the $pe^+e^-$ missing energy of the system was 
...
\missingfigure[figwidth=3in]{This will be figure 15 from analysis note.}\label{fig:sys}
...
\bibitem{HM}X.~Ji, Phys.\ Rev.\ Lett.\ \textbf{78}, 610 (1997); 
	...
	D.~Muller, {\it et al.} 
	%D.~Robaschik, B.~Geyer, F.~M.~Dittes and J.~Horejsi,
          Fortsch.\ Phys.\  {\bf 42}, 101 (1994).
%	M.~Diehl, T.~Feldmann, R.~Jakob, and P.~Kroll, Eur.\ Phys.\ 
%	J.\ C\ \textbf{8}, 409 (1999).
...
\bibitem{PDG} C.~Patrignani \textit{et al}. (Particle Data Group), Chin.\ 
...
%\bibitem{GlueX} \textit{et al.} (GlueX Collaboration), submitted to Phys.\
%        Rev.\ Lett.\
