...
\title{Photoproduction of $\pi^0$ on Hydrogen with CLAS}
...
\vskip 0.2in
We report the first high precision measurement of \textcolor{red}{the} exclusive $\pi^0$ 
photoproduction cross section via Dalitz decay and \textcolor{red}{$e^+e^-$ pair} conversion mode on a 
hydrogen target in a wide kinematic range with the CLAS 
setup at Thomas Jefferson National Accelerator Facility. 
The measurement \textcolor{red}{was} performed in the reaction $\gamma p\to 
...
\textcolor{red}{were} detected while the photon was not detected. The $\pi^0$ is 
identified by analyzing the missing mass of proton.  This new data \textcolor{red}{sample} quadrupled 
...
valid within the full angular range \textcolor{red}{($\theta = 
...
how Regge phenomenology works for the full CLAS energy range \textcolor{red}{of 
...
\textcolor{red}{Previous bremsstrahlung measurements, for $2~\leq E\leq 
...
\textcolor{red}{A significant increase in the 
...
for the $\gamma p\to\pi^0p$ reaction above the "resonance" regime.
\textcolor{red}{The new measurement presented here is the only measurement that can bridge resonance and 
high-energy (``Regge") regimes for $\pi^0$ photoproduction.}

\textcolor{red}{This unique data set quadruples the world bremsstrahlung 
database above the resonance range, E = 2~GeV, and also constrains the high energy 
phenomenology well with previous a CLAS measurement~\cite{du07}.}	
	
%This unique opportunity increases the
%available database above the resonance range, E = 2~GeV, by a significant amount.}
%As will be seen, this data set quadruples the world bremsstrahlung 
%database above E = 2~GeV and constrains the high energy 
%phenomenology well with previous a CLAS measurement~\cite{du07}..
%\textbf{Experiment:} 
The experiment was performed \textcolor{red}{ during March-June 2008 }with the CLAS setup 
...
liquid hydrogen target. \textcolor{red}{The experiment as a whole was a set of different experiments running at the same time with the same experimental configuration (cryogenic target, tagger, trigger configuration and
CLAS) and was designated with the name ``g12''. Particle identification for the experiment was based on $\beta$ vs. momentum$\times$charge.} The experimental details are given 
...
%----------------------------------------------------------
%\textbf{Data analysis:} 
Lepton identification was based on conservation of \textcolor{red}{the $\pi^0$ mass}. Once 
...
mass of $e^+e^-(\gamma)$=m$^2_{\pi^0}$. \textcolor{red}{The values of the ``confidence levels'' cuts employed was determined using statistical significance to get the best signal/background ratio.}  The ``confidence levels''
for each constraint were consistent between g12 data 
and Monte-Carlo simulations. \textcolor{red}{Monte-Carlo generation was performed 
...
ratio, fiducial cut, and the z-vertex cut.  \textcolor{red}{The largest contributions to the systematic uncertainties 
were the sector-to-sector efficiency (4.4-7.1\%), flux determination (5.7\%) and the cut on the 1-C fit pull probability (1.6-6.1\%). All systematic uncertainties and there determinations are described in~\cite{Kunkel}.} 
%--------------------------------------------------
\begin{figure}[htb!]
\centerline{
...
The new CLAS high statistics cross sections, 
presented here, for $\gamma p\to\pi^0p$ are compared in 
Figs.~\ref{fig:scaling} and \ref{fig:t_data} with 
data from previous \textcolor{red}{CLAS measurements}~\cite{du07}, and 
...
particularly with the previous  \textcolor{red}{CLAS data}.
%----------------------------------------------------
\begin{figure*}[htb!]
\centerline{
...
\centerline{
	...
	\textcolor{red}{(a) $\theta$ = 50$^\circ$,
		(b) $\theta$ = 70$^\circ$, 
		(c) $\theta$ = 90$^\circ$, and
		 (d) $\theta$ = 110$^\circ$}
	     in the c.m. frame as a function of c.m. 
		energy squared, $s$. The red filled circles are 
	    the current $g12$ data. The recent tagged data 
		are from previous  \textcolor{red}{CLAS measurements}~\protect\cite{du07} (black 
		...
		\textcolor{red}{PR15} solution (no new CLAS data are used for the 
...
\centerline{
		...
		current CLAS $g12$ (red filled circles) and a previous  \textcolor{red}{CLAS 
		...
		each panel. The uncertainties plotted are only the statistical  \textcolor{red}{uncertainties}. 
		...
		\textcolor{red}{(a) E = 2825~MeV and W = 2490~MeV,
...
with energy) agrees with the presented CLAS data.   \textcolor{red}{This is surprising since
...
$|t|\sim$5~GeV$^2$,  \textcolor{red}{where the Regge models~\cite{Goldstein,Laget,Donnachie} predict incorrect 
...
5.0$~GeV$^2$,  which agrees with the data.  
 \textcolor{red}{NOTE FROM MK. REMOVED THE PARAGRAPH AFTER THIS AS SUGGESTED BY COMMITTEE. ONLY DID THIS TO SHOW WHAT PAPER LOOKS LIKE WITHOUT IT. TEX FILE HAS PARAGRAPH COMMENTED OUT}
%That is why 
...
\textcolor{red}{The results presented in this paper form part 
...
\bibitem{g12} G12 experimental group, CLAS-NOTE 2017 - 002, 2017 \url{https://misportal.jlab.org/ul/Physics/Hall-B/clas/viewFile.cfm/2017-002.pdf?documentId=756}.

\textcolor{red}{\bibitem{Kunkel} M.~C.~Kunkel,  CLAS-NOTE 2017 - 005, 2017 \url{https://misportal.jlab.org/ul/Physics/Hall-B/clas/viewFile.cfm/2017-005.pdf?documentId=767}.}
\textcolor{red}{\bibitem{PLUTO} I.~Frohlich {\it et al.}, %``Pluto: A Monte Carlo Simulation Tool for Hadronic Physics,'' 
...
%[21] D. Hornidge et al. (A2 and CB-TAPS Collaborations), Phys. Rev. Lett. 111, 062004 (2013); 
\textcolor{red}{\bibitem{mami}P.~Adlarson \textit{et al.}  (A2 Collaboration at MAMI),
